...
除了传统的 CLI 命令，你可以直接通过与 AI 对话的方式生成测试报告，这是最自然、最高效的方式：

#### 基本对话示例

```
用户: 测试数据@test_account_pool.json，打开项目 https://localhost:3000，
      帮我生成 https://localhost:3000/admin/profile 页面的 Allure 测试报告，
      使用 4 个 worker 执行
```

AI 将自动完成以下工作：
1. 🔍 **分析页面结构** - 打开目标 URL，识别页面元素和交互
2. 📝 **生成 Page Object** - 创建 `pages/profile_page.py`
3. ⚡ **生成测试用例** - 创建 `tests/test_profile.py`
...
| 指令类型 | 示例 |
|----------|------|
| **生成测试报告** | "帮我生成 https://example.com/login 页面的测试报告" |
...
```
# 指定特定测试文件
用户: 运行 tests/test_profile.py 的测试，生成报告

# 只运行 P0 测试
用户: 只运行 P0 级别的测试用例

# 调试模式
用户: 用 headed 模式运行测试，我要看浏览器执行过程

# 修复登录问题
用户: 重新执行，检查如果已登录，退出登录后重新进行
```
...
生成后的文件:
├── pages/
│   ├── login_page.py        # 登录页面对象（如需登录）
│   └── profile_page.py      # 目标页面对象
├── tests/
│   └── test_profile.py      # 测试用例（P0/P1/P2 分级）
├── allure-results/          # 测试结果数据
└── allure-report/           # HTML 测试报告
```
